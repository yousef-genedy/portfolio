...
export default function PostCard({ post }: PostCardProps) {
  return (
    <article className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 transition-colors hover:border-zinc-700">
      <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">{post.date ?? "Draft"}</p>
      ...
      <Link href={`/posts/${post.slug}`} className="mt-4 inline-block text-sm text-zinc-200 hover:text-sky-300">
        Read post
      </Link>
    </article>
  );
